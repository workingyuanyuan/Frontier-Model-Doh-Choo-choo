 ...
 * LARGEST quality-feasible scale `N` that reaches `M`. A quality share cap
 * means deleting a healthy component can make a subset infeasible, so the
 * search must inspect every feasible scale without assuming monotonicity.
 *
 * `M` is the user-facing axis. Emit every count reachable on either curve.
 *
...
export const buildDisplaySetFromCurves = (
  ...
  const notes = [
    ...(unconstrained.benchmarkQuality
      ? [
          `Quality policy reviewed ${unconstrained.benchmarkQuality.reviewedAt}: excluded ${unconstrained.benchmarkQuality.excludedBenchmarkIds.join(', ')}; limited ${unconstrained.benchmarkQuality.limitedBenchmarkIds.join(', ')} to at most 1/${unconstrained.benchmarkQuality.minOtherBenchmarksPerLimited + 1} of each primary dimension. Largest-scale selection applies only to quality-feasible subsets.`,
        ]
      : []),
    `Generated by "pnpm data:generate-display-set" from the coverage report, reference date ${unconstrained.referenceDate}. Do not hand-edit: rerun the command.`,
...
export const generateDisplaySet = async (
  ...
  const outputPath = resolve(
    options.outputPath ??
      join(repoRoot, 'data', 'mappings', 'display-set.json'),
  );
  ...
  validateDisplaySet(
    generated.displaySet,
    workspaceData.benchmarkMapping,
    policy.benchmarkQuality,
  );

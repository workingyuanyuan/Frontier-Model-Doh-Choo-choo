...
- **五維能力拆解**  
  將模型能力細分為推理（Reasoning）、知識（Knowledge）、程式碼（Coding）、智慧體（Agentic）、語言（Language）等五大領域。數學與長上下文推理歸入推理，輸出限制的遵循歸入語言。
- **成本與效能分佈**  
  結合任務呼叫成本（USD / 任務或百萬 Token 價格）與綜合分數，直觀呈現各前沿模型的性價比位置。
- **原始資料追溯**  
  每項評測數據均記錄來源網址、採集時間與原始快照內容雜湊（SHA-256），確保所有資料來源公開透明、可供查核。
- **推理強度分級 (Reasoning Effort)**  
  針對支援思考推理（Reasoning / Thinking）的模型，依據不同的推理強度（`non-reasoning < low < medium < high < xhigh < max`）個別記錄與評分，避免混淆不同運算設定下的表現。
- **嚴謹顯示門檻與開發者檢視**  
  主榜僅展示通過完整評測項目門檻（Display Set）的模型；同時提供開發者檢視模式，可隨時查看測試項目未齊全的模型以及其缺少的測試項目清單。
...
| **成本與效能分佈圖 (Quality vs. Cost)** | 互動式散佈圖，支援對數座標切換與不同基準任務切換，方便觀察模型能力表現與呼叫成本的對應關係。             |
...
├─────────────┬─────────────┬─────────────┬─────────────────┤
│ 推理        │ 知識        │ 程式碼      │ 智慧體          │
│ (Reasoning) │ (Knowledge) │ (Coding)    │ (Agentic)       │
├─────────────┴─────────────┴─────────────┴─────────────────┤
│ 語言 (Language)                                           │
└───────────────────────────────────────────────────────────┘
```
...
| [維度映射手冊 (BENCHMARK_DIMENSION_MAPPING.md)](docs/BENCHMARK_DIMENSION_MAPPING.md) | 各項評測集與五大能力的映射定義與注意事項 |

...
本專案整合多個第三方評測來源，將模型表現拆解為五項核心能力維度，並結合實際呼叫成本，提供直觀的評測數據與分析視圖。
...
- **五維能力拆解**
  將模型能力細分為推理（Reasoning）、知識（Knowledge）、程式碼（Coding）、智慧體（Agentic）、語言（Language）等五個領域。數學與長上下文推理歸入推理，輸出限制的遵循歸入語言。
- **成本與效能分佈**
...
| **開發者檢視模式 (Developer Mode)**     | 開啟後可查看因測試項目未齊全而未列入主榜的模型與缺少的評測項目，以及只缺一個維度的 Profile 清單。 |

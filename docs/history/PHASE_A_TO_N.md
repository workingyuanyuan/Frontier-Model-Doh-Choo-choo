...
- `docs/PROJECT_HANDOFF.md` §8「待使用者決定」→ 標記為已決定，指向 `docs/SPEC.md`
...
- `docs/README.md` → 把 `history/REFACTOR_SPEC.md` 標為 Historical、`SPEC.md` 標為 Current／Binding
...
審核證據：[docs/history/GATE1_AUDIT.md](GATE1_AUDIT.md)——18 個 benchmark 各抽三列共 54 筆，全部屬於會出現在主畫面的模型；52 筆逐位相符、0 筆不符，另 2 筆是出處指錯頁面（已修）。
...
使用者判讀 [docs/COVERAGE_MATRIX_REPORT.md](../COVERAGE_MATRIX_REPORT.md) 後，選定報告中 N=15 的最佳組合**減去 `aa-briefcase`**，共 14 項。`aa-briefcase` 一項就擋掉 DeepSeek V4 Pro、GPT-5.6 Luna 與 GPT-5.6 Terra；移除後完整模型數由 11 升到 14，維度仍為 8/8，四個來源全部保留。
...
**不得**修改 `docs/SPEC.md`；文件對齊規格，不是反過來。
...
- `docs/SPEC.md:119`：「每個其他來源投一票（**該來源對這個模型發布過的最高具名檔位**），得票最多的檔位勝出；平手時取較高的檔位。」
...
1. `docs/history/STAGE5_PLAN.md`、`docs/history/STAGE5_TODO.md`：在檔首加歷史標記，明確指出它們描述的是
   ...
   `docs/SPEC.md` 與 `tasks/claude-code-plan.md`。**不刪除內容**，只加標記。
...
H5 於 2026-08-21 判定**通過**（[驗收報告](F2_ACCEPTANCE.md)），
...
`docs/SPEC.md` §6.1／6.2／6.3／6.4 已同步改寫：
...
規格 `docs/SPEC.md` §6.3 已補上「軸的縮放規則」與「進階圖的序列可見性」。
...
規格已改寫，見 `docs/SPEC.md` §6.3 的「進階圖」「進階圖的軸定義」「為何 Y 軸不能
...
裁決後寫進 `docs/SPEC.md` 對應章節（N1 的第一步）。
...
- **統計不得混入凍結來源。** `SPEC.md` §3.2 凍結的六個目錄（`lech-writing`、
...
`SPEC.md` §5.2.1 與 §5.3（權威在規格，不在提案文件）：

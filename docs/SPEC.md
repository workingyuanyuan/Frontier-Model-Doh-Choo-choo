...
將現役公開來源的模型量測整合為五維靜態看板；同一集合的模型使用相同 benchmark 計分，通過完整矩陣與五維門檻後展示，並可逐筆追溯原始證據。
...
- **維度（dimension）**：五個能力軸之一，例如 Coding。
- **顯示清單（display set）**：一組固定的 benchmark ID。模型必須在這組清單的**每一項**上都有分數，才會出現在主畫面。

## 2. 重構分期紀錄

以下保留當時的分期與審核安排。常態更新的現行觸發與完成條件見 [操作手冊](OPERATIONS.md)。
...
- 缺任何一項 → 不進主畫面，進開發者模式。
- 主畫面因此**不會出現 N/A**。
- 顯示清單由核准政策生成；每次刷新先重產集合再重建產品，見 [計分方法](SCORING_METHODOLOGY.md) 與 [操作手冊 §3](OPERATIONS.md#3-重產集合與建立目前版本)。
...
**狀態**：已實作。以下保留 R1 起的設計裁決；2026-09-06 確認每次刷新依既有核准政策重產集合，影響判斷與審核依 [操作手冊](OPERATIONS.md)。
...
**R2（現行）**：集合選擇政策由使用者核准；代理每次刷新依該政策重新產生 preset 組成，並比較展示、分數與排名差異。政策變更仍需使用者裁決。
...
**授權演進**：D-N10-2 當時只允許修改 schema、外殼與 notes。2026-09-06 已將依核准政策重產 `benchmarkIds` 納入每次刷新；代理仍不得自行改變政策或手選組成。
...
報告供判讀集合取捨與刷新影響。依核准政策生成 `display-set.json` 是常態更新的一部分；政策變更交由使用者裁決。
...
右側 Models 控制採兩層結構：模型 checkbox 開啟或關閉該模型的全部 effort，個別 effort 按鈕
另可獨立開關。每列顯示「目前可見 effort／該模型完整 ProductVersion effort 母體」；不通過目前
來源交集的 effort 保持可見但不可用，來源關閉後若恢復資格即自動成為可用點。部分開啟時原生
...
R21 後模型 checkbox 仍保留上述整體序列控制，並在其下增加每個 effort 的原生按鈕；任一 effort
關閉後，該點與相鄰線段不畫，也不參與軸域。checkbox 在部分 effort 可見時使用 indeterminate
狀態。來源與 effort 兩層控制都必須可由鍵盤操作，且不得把 button 巢狀放進 checkbox 的 label。
...
現行架構邊界見 [ARCHITECTURE.md](ARCHITECTURE.md) 與 [REFACTOR_DISCARD_LIST.md](REFACTOR_DISCARD_LIST.md)。產品資料需遵守：
...
- 不重寫或刪除凍結的來源資料。
- 發布授權、資料提交與審核程序見 [OPERATIONS.md](OPERATIONS.md)。
...
現行程序、每次依核准政策重產 benchmark 集合、模型與分數排名比較、人工抽查格式及審核交接，統一見 [OPERATIONS.md](OPERATIONS.md)。它同時適用定期更新、新模型更新與擷取／計分修正。

刷新報告保存於 `docs/refresh/<YYYY-MM-DD>.md`；內容與完成條件由操作手冊 §6 維護。

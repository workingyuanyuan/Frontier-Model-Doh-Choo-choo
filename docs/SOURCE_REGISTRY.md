...
- Current implementation state: the bounded one-page CLI remains available for smoke tests. `pnpm ingest:livebench:all` now orchestrates the declared dataset sequentially through the tested non-overlapping page plan, rejects a changing total row count and stops on the first failed page. Hub revision capture, alias resolution, aggregation and publication remain gated work.

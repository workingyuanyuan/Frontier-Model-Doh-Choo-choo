...
| LegalBench                | Knowledge, Language     | Covers issue spotting, rule application, legal conclusions, interpretation and rhetorical reasoning. | It is a broad legal-reasoning suite, not a legal-research agent; preserve task category and grader.               |
...
| MedCode              | Context, Instruction    | Applies ICD-10-CM coding knowledge to full hospitalization records. | Preserve primary versus secondary diagnosis scoring, record scope and coding-standard version.                |
| TaxEval v2           | Reasoning, Math         | Tests hard tax-law answers against expert ground truth.             | Answer correctness and stepwise reasoning are separate tasks; preserve tax year, judge and benchmark version. |
...
| MedScribe                                  | Context, Instruction, Knowledge | Converts clinical conversations into structured SOAP notes. | Preserve transcript generation method, note template, rubric and judge-alignment version. |
...
| FrontierCode (`frontier-code-1-1`) | Agentic, Context        | Cognition's production-quality code benchmark, scored with a weighted rubric over model+harness runs. | Cognition FrontierCode and Proximal FrontierSWE are different benchmarks from different organisers. Never merge them or reuse the `frontierswe` id for Cognition scores. |
...
| IOI                                | Reasoning, Math         | Competitive programming problem solving with executable submissions and subtask grading.              | Preserve year, submission budget, tool harness and scoring metric.                                                                                                       |
| Code Migration                     | Agentic, Context        | Reimplements existing programs across languages under hidden behavioral tests.                        | Migration direction, source-program scope, target language, anti-cheat checks and test suite matter.                                                                     |
| Vibe Code Bench                    | Agentic, Context        | Measures whether a model can build working multi-file web applications from a prompt.                 | Preserve Vals version and evaluation harness; do not merge it with SWE-bench.                                                                                            |
...
| Finance Agent v2           | Knowledge, Coding, Context       | Measures agent completion of core financial-analyst workflows.                                | It is a Vals-owned task, not GDPval; preserve its version and execution environment.                                                     |
...
| SkillsBench                | Coding, Instruction              | Compares the same software agent with and without reusable task-specific skills.              | Preserve agent scaffold, skill condition, task suite and repeated-run aggregation.                                                       |
| Harvey Legal Agent         | Context, Knowledge, Instruction  | Produces legal work with file-system tools and document, spreadsheet and presentation skills. | Preserve public versus held-out set, artifact skills, criteria pass rate and strict task-resolution score.                               |
| Excel Modeling Benchmark   | Math, Context, Instruction       | Builds usable Excel financial models from prompts and source files.                           | Template and scratch modes, numerical/formula/presentation rubrics and workbook behavior are distinct.                                   |
| Legal Research Bench       | Knowledge, Reasoning, Context    | Uses legal-search and document-retrieval tools to synthesize supported answers.               | Preserve practice-area mix, tool environment, expert rubric and strict all-pass metric.                                                  |
| Public Benefits Bench      | Knowledge, Language, Instruction | Uses search and multi-turn questioning to navigate location-dependent SNAP rules.             | Preserve the four evaluation conditions and benchmark version; raw-model knowledge is not interchangeable with the both-tools condition. |
| CyberBench                 | Coding, Reasoning                | Reproduces and patches real OSS-Fuzz vulnerabilities end to end.                              | PoC and Patch are separate tracks; preserve harness, crash oracle, compile checks and hidden holdouts.                                   |
| ReverseEngBench            | Reasoning, Coding, Context       | Uses professional shell tools to recover semantics from protected binaries without source.    | Preserve task suite, protection settings, toolkit and fully-solved versus capability-score metrics.                                      |
...
| CorpFin v2                     | Reasoning, Knowledge    | Measures long-context understanding of credit agreements and corporate-finance documents. | Keep the Vals version explicit; do not substitute a generic finance-knowledge score.   |

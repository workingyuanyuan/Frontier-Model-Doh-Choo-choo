...
- Edition activation is serialized by a PostgreSQL transaction lock and constrained to one active row by a partial unique index. Formal activation revalidates the scoring method and every ranked entry; incomplete snapshots may activate only in explicit `PREVIEW` mode. Switching and rollback update the active pointer without rewriting snapshots, and each state change appends a SHA-256 audit entry chained to the prior audit hash. Idempotent retries are no-ops and do not add duplicate audit events.

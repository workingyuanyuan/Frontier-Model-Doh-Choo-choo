...
1. **排行榜**：總分、八維分數、排序、搜尋、思考強度選擇，以及列內展開的模型明細（見 6.2）。
2. **八維雷達圖**。
3. **性價比圖表**（兩張，見 6.3）。
...
- **展開內容不刪減**：八維、逐 benchmark 分數與可點開的出處（§7）全部留在列內。
...
1. **Y 保持是一個「分數」。**軸標題寫得出實際範圍（實測 16.9–61.6），符合下方「軸的縮放規則」
...
（Overall Score）與 X（成本指數）、進階圖的 Y（三來源平均分數）與 X（三來源成本指數）。

2026-08-21 進階圖改為聚合圖後，它的 X 從 USD 變成與預設圖同單位的成本指數（見上方「進階圖
的軸定義」），因此兩張圖的 X 軸可以直接對照解讀；但兩者的權重組成不同（四來源等權 vs. 三
來源各 1/3），數值不等價，軸標題必須各自寫出自己的實際範圍。

...
## Q4 — 成本圖來源控制列與 Models 箭頭對齊

狀態：完成

**目的**：將預設權重說明與 Advanced 來源按鈕移至模式切換左側，並讓 Models 展開箭頭與標籤保持水平置中。

**範圍**：`apps/bench` 的成本圖 header DOM、控制列樣式與對應測試。不改動圖表資料、來源資格、計分或座標算法。

**驗收**：Default 模式由左至右顯示權重說明、Advanced、Models；Advanced 模式由左至右顯示四個來源按鈕、Default、Models；Models 箭頭與文字垂直置中且正確反映展開狀態；行動版無水平溢位，完整基準驗證依規定順序通過。
